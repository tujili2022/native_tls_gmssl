...
use self::gmssl::error::ErrorStack;
use self::gmssl::hash::MessageDigest;
use self::gmssl::nid::Nid;
use self::gmssl::pkcs12::Pkcs12;
use self::gmssl::pkey::{PKey, Private};
use self::gmssl::ssl::{
    self, MidHandshakeSslStream, SslAcceptor, SslConnector, SslContextBuilder, SslMethod,
    SslVerifyMode,
};
use self::gmssl::x509::{store::X509StoreBuilder, X509VerifyResult, X509};
use std::error;
use std::fmt;
use std::io;
use std::sync::Once;
...
impl TlsConnector {
    pub fn new(builder: &TlsConnectorBuilder) -> Result<TlsConnector, Error> {
        ...
        connector.set_ciphersuites("GMTLS_SM2DHE_WITH_SMS4_SM3:GMTLS_SM2_WITH_SMS4_SM3").unwrap();
